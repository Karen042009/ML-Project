...
def compare_last_rows():
    ...
    if not misclassified.empty:
        ...
        print(misclassified[["ip_address", "true_label", "is_bot_prediction", "bot_probability"]].head())
    else:
        print("\nAll 255 rows were predicted correctly!")
if __name__ == "__main__":
    if not os.path.exists(names.FINAL_MODEL_PATH):
        print("Error: Model not found. Please train first by running: python3 main.py (with train() called)")
    else:
        compare_last_rows()
