...
def get_data(file_path="data/visits.csv"):
    try:
        ...
        df = df_raw.copy()
        print(f"'{file_path}' ֆայլը հաջողությամբ բեռնվեց։")
        return df
    except FileNotFoundError:
        print(f"Սխալ։ '{file_path}' ֆայլը չի գտնվել։")
        sys.exit()
...
def run_visualizations(dataframe):
    ...
    if "timestamp" in dataframe.columns:
        ...
        dataframe["day_of_week_name"] = dataframe["timestamp"].dt.day_name()
        dataframe["date_only"] = dataframe["timestamp"].dt.date

    if "browser" in dataframe.columns:
        plt.figure(figsize=(12, 7))
        plt.title("Թոփ 10 ամենատարածված բրաուզերները", fontsize=16)
        sns.countplot(
            y="browser",
            data=dataframe,
        ...
        plt.xlabel("Այցելությունների քանակ")
        plt.ylabel("Բրաուզեր")
    ...
    if (
        ...
        plt.title("Այցելությունների բաշխումն ըստ սարքատեսակի", fontsize=16)
        plt.pie(
            device_counts,
    ...
    if "path" in dataframe.columns:
        ...
        plt.title("Թոփ 10 ամենաշատ այցելվող էջերը", fontsize=16)
        plt.xlabel("Այցելությունների քանակ")
        plt.ylabel("Էջի հասցե (Path)")
    ...
    if "referrer" in dataframe.columns:
        ...
        if not referrer_data.empty:
            ...
            plt.title('Թոփ 10 ռեֆերալ աղբյուրները (առանց "direct")', fontsize=16)
            plt.xlabel("Այցելությունների քանակ")
            plt.ylabel("Աղբյուր")
    ...
    if "hour" in dataframe.columns:
        ...
        plt.title("Այցելությունների ակտիվությունն ըստ օրվա ժամերի", fontsize=16)
        plt.xlabel("Օրվա ժամ (0-23)")
        plt.ylabel("Այցելությունների քանակ")
    ...
    if "day_of_week_name" in dataframe.columns:
        ...
        plt.title("Այցելությունների ակտիվությունն ըստ շաբաթվա օրերի", fontsize=16)
        plt.xlabel("Շաբաթվա օր")
        plt.ylabel("Այցելությունների քանակ")
    ...
    if "date_only" in dataframe.columns:
        ...
        plt.title("Այցելությունների քանակի դինամիկան ըստ օրերի", fontsize=16)
        plt.xlabel("Ամսաթիվ")
        plt.ylabel("Այցելությունների քանակ")
        plt.grid(True)
        plt.xticks(rotation=45)
    ...
    if "is_bot" in dataframe.columns:
        ...
        plt.title('Դասերի անհավասարակշռությունը ("is_bot")', fontsize=16)
        plt.ylabel("Այցելությունների քանակ")
        plt.xticks(
            [0, 1],
            labels=[
                f"Ոչ բոտ ({target_counts.get(False, 0)})",
                f"Բոտ ({target_counts.get(True, 0)})",
            ],
        )
    ...
    if all(col in dataframe.columns for col in important_cols):
        ...
        g.fig.suptitle(
            'Հատկանիշների Զույգային Կապերը Ըստ "is_bot"-ի Դասի', y=1.02, fontsize=16
        )

        new_labels = ["Ոչ բոտ", "Բոտ"]
        for t, l in zip(g._legend.texts, new_labels):
            t.set_text(l)

        g._legend.set_title("Դաս")
